...
| 定向 | 工具面全集计数+名单断言（185）、注册唯一写点静态锚、抽 5 个代表工具（file.read/vault.get/shell.exec/market.status/tool.help）直呼契约绿 |
| 构建 | `:app:assembleDebug` 绿 |
| 真机 L3 | 平板冒烟：对话往返 + 一次写类审批卡（悬浮卡正常）+ 一次工具直呼（tool.help）——截图+logcat 含时间戳 |

### 销项条件（下列全满足）

- [ ] 工具注册面搬入独立模块（MainActivity 内 mcpHandlers[ 直接写入=零，只剩装配调用）
