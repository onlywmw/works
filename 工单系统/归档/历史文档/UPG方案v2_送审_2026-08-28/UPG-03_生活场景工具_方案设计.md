...
1. **L4 接线（本单核心，v1 全文未提）**：MainActivity.kt:3121 循环后追加 `SceneTools.all()` 迭代（复用 :3159-3173 通用分支）+ 同步 `toolParamSchemas`（rebuildAgentTools 消费点 :5660，缺 schema 则 AI 盲调）；:8389 暴露循环（:3192-3201）自动跟随
