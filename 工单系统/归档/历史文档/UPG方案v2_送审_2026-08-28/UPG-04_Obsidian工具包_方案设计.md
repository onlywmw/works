...
| `obsidian.vault.rescan` | 重建索引 | **ASK**（v2 修正：老版实为写类 plan——rescan 改写登记，ObsidianToolProvider.java:236/254；v1 标 harmless 是审批静默降级） |
...
2. SAF 通路：detect=引导 ACTION_OPEN_DOCUMENT_TREE → register=takePersistableUriPermission+prefs 落盘（单写点）→ DocumentFile 读写；**沙盒内核移植老版 ObsidianVault.resolve**（逐段 canonical+symlink 检测 :135-164），沙盒根=已授权 vault（非 file.* 的 Download/MOV 沙盒）
