 ...
 * - 放行 outcome 判定单源 = [isGranted]（only-once 只认 allowed-once；其余工具 once/turn/remembered/plan 放行）
 *
 * UPG-68 安全语义零改动：only-once / fail-closed / 豁免顺序（turn→goal→remembered）原样保留。
 * UPG-76 预审单执行绑定：consumeIfApproved 插入在 remembered 之后、FIFO 弹窗之前（命中放行+扣减 / 未命中转新 ASK）。
 */
class ApprovalService(
    ...
    companion object {
        ...
        /** UPG-76：预审单执行绑定命中（PlanApprovalStore §8.3 已批准步骤——放行 + 扣减，跳过弹窗）。 */
        const val OUTCOME_ALLOWED_PLAN = "allowed-plan"
         ...
         * turn/remembered/plan 豁免放行。goal outcome 不在放行列（与 UPG-68 收口语义精确等价，不改动）。
         * UPG-76：allowed-plan（预审单绑定命中）计入放行面——only-once 永不预批，故 its 分支保持 once-only。
         */
        fun isGranted(toolName: String, outcome: String): Boolean {
            val onceOk = outcome == OUTCOME_ALLOWED_ONCE
            val otherOk = outcome == OUTCOME_ALLOWED_TURN
                || outcome == OUTCOME_ALLOWED_REMEMBERED
                || outcome == OUTCOME_ALLOWED_PLAN
    ...
    /**
     * UPG-76：预审单批准清单（PlanApprovalStore §8.3——执行绑定查询只此一处消费）。
     * request() 在 remembered 豁免之后、FIFO 弹窗前查 consumeIfApproved：命中放行 + 扣减（跳过弹窗）；
     * only-once / MONEY 由 Store 直返 MISS（fail-closed 转 ASK），双保险不入簿不吃清单。
     * null = 未装配（无预审单机制，行为与 UPG-75 完全一致）。
     */
    @Volatile
    var planStore: PlanApprovalStore? = null

    /**
     * UPG-76 触发口径协调钩子（对话面 runChat 内由宿主装配）：首个审批级 ASK 到达 request（remembered 之后、
     * 清单绑定之前）时宿主暂停本调用 → 跑「计划补全轮」（READ-only 面）→ ≥2 审批级出单 / 恰好 1 个回退现行。
     * 宿主自管 run 作用域、重入防护与 MCP 面隔离（MCP 直调时钩子必须为空）。钩子只「出单不裁决」——
     * 裁决仍由下方 consumeIfApproved 按清单节点统一完成（本调用是否放行取决于它在单上被批还是被否）。
     * null（MCP 面 / 未装配）= 无预审单编排，行为与 UPG-75 完全一致。MONEY / only-once 永不经此钩子。
     */
    @Volatile
    var preApprovalPlanner: (suspend (PendingInfo) -> Unit)? = null
    ...
    suspend fun request(toolName: String, reason: String?, args: Map<String, Any?>): String {
        ...
        // UPG-76 触发口径（宿主 runChat 内装配，仅对话面）：首个审批级 ASK 到此处（remembered 之后）→
        // 暂停跑计划补全轮出单。MONEY/only-once 永不经此（MONEY 永远实时逐笔；only-once 不吃清单不吃豁免）。
        // 钩子只出单不裁决——本调用是否放行由下方 consumeIfApproved 按单上节点统一裁决（批→HIT / 否→DENIED）。
        if (!onlyOnce && !MoneyTools.isMoney(toolName) && preApprovalPlanner != null && planStore != null) {
            preApprovalPlanner!!.invoke(PendingInfo(requestId, toolName, reason, args))
        }

        // UPG-76 预审单执行绑定：remembered 之后、FIFO 弹窗前查 PlanApprovalStore——
        // 命中（清单内 approved + 未过期 + 次数未耗尽）→ 放行 + 原子扣减，跳过弹窗；outcome=allowed-plan。
        // 已决否（REJECTED/BLOCKED 节点——单上被拒/被阻断）→ outcome=rejected，不重复弹窗（「阻断下游」语义）。
        // 未命中（计划外 / 已耗尽 / 已过期 / 未装配）→ 不拦截，落下方 FIFO = 当场转新 ASK（用户实时决策）。
        // only-once / MONEY 不入簿不吃清单：Store.consumeIfApproved 首行即对它们直返 MISS（双保险，入簿侧已拒）。
        when (val c = planStore?.consumeIfApproved(toolName, args)) {
            PlanApprovalStore.Consume.HIT -> {
                android.util.Log.i("ApprovalService", "预审单绑定命中: $toolName")
                session.append("approval/asked", mapOf("requestId" to requestId, "toolName" to toolName, "reason" to "预审单已批准"))
                session.append("approval/decided", mapOf("requestId" to requestId, "outcome" to OUTCOME_ALLOWED_PLAN))
                if (openedTurn) closeTurn(session, turn)
                return OUTCOME_ALLOWED_PLAN
            }
            PlanApprovalStore.Consume.DENIED -> {
                android.util.Log.i("ApprovalService", "预审单已决否（阻断下游）: $toolName")
                session.append("approval/asked", mapOf("requestId" to requestId, "toolName" to toolName, "reason" to "预审单上该步未获批准"))
                session.append("approval/decided", mapOf("requestId" to requestId, "outcome" to OUTCOME_REJECTED))
                if (openedTurn) closeTurn(session, turn)
