...
// UPG-78 A1：生成器测试输入自供电（构建图内消解断链）——每次 testDebugUnitTest 前先盘点在面工具全集。
//   collect 输出 app/build/inventory/tools.txt（gitignored）；ApprovalRegistryGeneratorTest 依赖其存在且新鲜
//   （stale tools.txt = 新 handler 漏盘点 = 注册表漏登记不红，A 方案保证每跑刷新；测试内 ProcessBuilder 兜底保留，防 IDE 直跑）。
val approvalInventoryCollect by tasks.registering(Exec::class) {
    group = "verification"
    description = "UPG-78 A1: 盘点在面工具全集（approval-inventory-collect.mjs）→ build/inventory/tools.txt（ApprovalRegistryGeneratorTest 输入）"
    workingDir = layout.projectDirectory.asFile
    commandLine("node", "../scripts/approval-inventory-collect.mjs")
}
// AGP 变体测试任务注册晚于脚本顶层 → matching+configureEach 惰性挂接（任务出现即生效）
tasks.matching { it.name == "testDebugUnitTest" }.configureEach {
    dependsOn(approvalInventoryCollect)
}
