...
> **合并注记（用户拍板 @2026-08-29）：本单与 UPG-04 合并施工**（共享 MainActivity 装配区：一次接线/一次 Token 申报/一次验收），合并派单文本 `设计师\派单\UPG-02+04合并_派单_2026-08-29.md`；串行约束：占装配区期间 UPG-07 不开工。

> 设计人：设计师 ｜ 日期：2026-08-26（v1）/ 2026-08-28（v2）｜ 状态：✅ 方案 v2（溯源修正+用户拍板收口），待派单 ｜ 优先级：P0
> v2.1（2026-08-28 大神速览收口）：device.network 限零权限子集（信号强度列施工期预研）；schema 随 Provider 走（§五）。
...
| 域 | 工具 | 功能 | 权限级 | 性质 |
|---|---|---|---|---|
| 设备 | `device.network` | 网络类型/在线状态/是否计量（**零权限子集**；蜂窝信号强度细节 Android 10+ 需 ACCESS_FINE_LOCATION，列施工期预研，拿不到砍字段不砍工具） | harmless | 迁移 |
...
2. **注册四点配套，缺一工具不进面**：
   ...
   - `toolParamSchemas`——**schema 随 Provider 走**（DeviceProvider 自带 schemas 表，注册段一次 merge；不在 :177 静态硬编码人工同步，防 22+ 工具后遗漏——UPG-02/03/04 三单共享口径）；
