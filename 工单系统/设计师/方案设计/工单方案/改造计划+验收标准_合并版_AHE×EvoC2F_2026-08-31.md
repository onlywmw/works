...
门3 灰度：会话级 shadow 计数（只进规划上下文不执行→N 会话后转执行），退化自动回 shadow；**统计口径（2026-09-02 评审定稿，详见 专项_v2）**：按 skill_id+call_id 去重（bad_call=isError OR fabricate_hit，一次调用≤1 坏）+ 最小样本 MIN_CALLS=20 + 阈值 30%（安全止损非统计基线）+ 连续 3 坏熔断；会话仅观察窗口，回退对象=Skill；降级自动化/恢复人工
```
...
| **A2-4b** | 并发一致性 | 同 skill 10 signal 并发 + 重复 event_id | count=10；重复 event_id 不增（幂等） |
| A2-5a | 门 3 退化回退（机制） | 注入高 isError / fabricate_hit 信号会话 | 自动回 shadow + Ledger 记 DEGRADED/ROLLED_BACK（DEGRADED=检测/ROLLED_BACK=实际切换，分开记） |
| **A2-5b** | 退化回退（真实回路） | 真实 AgentLoop 产生真实 call→guard/error→skill_id→持久计数→rollback | 生产真实事件正确驱动（非注入模拟） |
