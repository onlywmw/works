# UPG-06 确定性防编造回补（FabricateGuard 先行，三件套拆批）· 方案设计 v4

> 设计人：设计师 ｜ 日期：2026-08-26（v3 修订 2026-08-29；v3.1 评审#2 收口 2026-08-29；v3.2 评审#3 收口 2026-08-29；v4 批 2 场景定案 2026-08-30）｜ 优先级：批 1 P0 / 批 2 P1 ｜ 状态：✅ 方案 v4 完成（评审 §七 + 溯源图 §八 + 评审#2 §九 + 评审#3 §十 + **批 2 场景定案 §十一** 五环闭环）；**批 1 已合 main @2026-08-29（4a84fba）**；批 2 = AcceptanceJudge B1 起步（观察层）+ GoalGate ✅已销项 @2026-08-30（用户确认，论证见 §十一.1）
> v4 修订说明（批 2 场景定案 @2026-08-30，用户拍板）：① **决策点 2 已定案**——AcceptanceJudge expected 来源 = **B1 用户指定标准起步**（观察层不拦截），B2 市场任务自带留待市场任务体系成熟再议（§十一.2）；② **决策点 1 已定案**——GoalGate 三候选场景全排除，**✅ 用户确认销项 @2026-08-30**（§十一.1）；③ 批 2 状态从「挂起」改为「B1 可起步 / GoalGate 待确认」，不再含糊挂起。
...
### 批 2（P1）· GoalGate + AcceptanceJudge（场景定案 @v4）

4. **GoalGate**：DeepTutor 教学场景移植件（单证据 0.5 封顶 / 0.9 过线）。**v4 场景定案（§十一.1）：三候选全排除 → ✅ 用户确认销项 @2026-08-30**（阈值不照搬——场景不存在则无从谈起；未来有场景可依 §十一.1 论证表重新立卡）。
5. **AcceptanceJudge 降级为观察层**（v2 修正；v4 expected 来源定案 = **B1 用户指定标准**）：老版 expected 实为 **journal 本地持**（`criteriaEvent` 落 append-only + `projectForModel()` 剔答案键），并非远程服务端；`pass()` 是 exact equals，只适用有确定答案的问答型任务。本批只做：journal 持 expected + 投影剔键 + 对拍结果落 journal（**不拦截、不重试**），仅问答型任务启用。expected 来源 **B1 起步**——用户显式指定（「按这份标准核对」形态，MOV 管家办事场景），**B2 市场任务自带留待市场任务体系成熟再议**（当前任务卡 schema 无此概念，引入成本高）。
...
2. ~~GoalGate 应用场景~~ **已定案 @v4（§十一.1）**：三候选场景全排除 → ✅ **销项**（用户确认 @2026-08-30）；
3. ~~AcceptanceJudge 的 expected 来源~~ **已定案 @v4（§十一.2）**：**B1 用户指定标准起步**（观察层不拦截），B2 市场任务自带留待市场任务体系成熟再议；
...
**批 2 启动前置（写入卡片，防空转）**：~~① GoalGate——用户确认新 MOV 存在「掌握度」交互场景；② AcceptanceJudge——问答型任务识别机制 + expected 来源（市场任务自带/用户指定）有定案。两条不满足，批 2 继续挂起。~~ **已定案 @v4（§十一）**：GoalGate 三候选全排除 → ✅ 销项（用户确认 @2026-08-30）；AcceptanceJudge expected 来源 = **B1 用户指定标准**，可起步。
...
## 十一、批 2 场景定案（v4，用户拍板 @2026-08-30）

> 起因：用户反馈「批 2 的设计不够完善，不知道怎么回答」——原决策点停留在机制层（掌握度场景/expected 来源），把决策负担甩给用户。本节省略：先把决策点翻译成产品语言（用户可感知的候选场景），再做选择题，每个选项配权衡 + 设计师推荐。

### 1. 决策点 1 · GoalGate「哪些交互算掌握」

**产品问题**：MOV 里有没有「AI 声称学会了/记住了某件事」的交互，值得事后核对？

| 候选场景 | 分析 | 权衡 | 结论 |
|---|---|---|---|
| A1 办事线（声称完成） | 已被批 1 FabricateGuard 覆盖（声称完成但零 tool_call） | 重复建设 | 排除 |
| A2 懂你线（声称记住偏好） | UPG-05 记忆显化页已交付，用户可直接看到记忆列表；加「掌握度」= 画蛇添足 | 重复且无增量 | 排除 |
| A3 教学线（教用户） | MOV 现状无任何「教/学」交互（AI 管家 = 办事 + 懂你，非教学产品） | 无场景 | 排除 |

**设计师结论**：三候选全排除 → **建议销项**（明确论证不适用，而非挂起待审——挂起只是把决定推迟到下一个无法回答的时刻）。
**状态**：✅ **用户确认销项 @2026-08-30**（三候选全排除，产品层无掌握度场景；老版 GoalGate.java 与论证表保留于 MOV-APP-old，未来若有场景可依本表重新立卡）。

### 2. 决策点 2 · AcceptanceJudge「expected 来源」

**产品问题**：什么时候你希望 MOV 的答复「对得上标准答案」？

| 候选 | 形态 | 成本 | 结论 |
|---|---|---|---|
| B1 用户指定标准（**推荐**） | 用户显式指定（「按这份标准核对」形态），MOV 输出结论时 journal 内对拍、不外发；正是管家办事场景 | 低，可起步 | **✅ 定案起步 @2026-08-30** |
| B2 市场任务自带 expected | 任务卡声明「本任务有标准答案」——当前市场任务卡 schema 无此概念 | 高，需先建任务卡 schema | 留待市场任务体系成熟再议 |

**设计师推荐**：B1 起步（维持观察层：journal 持 expected + 投影剔键 + 对拍落 journal，**不拦截不重试**）；B2 等市场任务体系成熟再评估。
**状态**：✅ **用户已确认 @2026-08-30**——B1 起步，B2 再议。

### 3. 批 2 结果状态

- **AcceptanceJudge**：B1 起步定案 → 可进入拆卡排期（观察层，P1，工作量小）；
- **GoalGate**：✅ **已销项 @2026-08-30（用户确认）**——自批 2 范围移除；若未来出现「掌握度/成效」场景，以本表论证为基重新立卡。批 2 剩余 = AcceptanceJudge B1 起步。
