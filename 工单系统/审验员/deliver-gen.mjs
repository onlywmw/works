...
function buildSkeleton(o, hashRes, tests, date, manifestInfo) {
  ...
  push("- （红线逐条对照待填）");
  push("");
  push("## 六之二、施工期重大回归与自纠（强制节——UPG-94 教训：自纠不报侵蚀信任链；无也必须填「无」，删节=打回）");
  push("");
  push("- （施工中是否发生过误删/误改既有功能并自纠？逐条：事实 + 影响面 + 自纠提交 + 是否已单独申报。无则填「无」）");
  push("- 视觉类追加变更（样式/尺寸/布局）：截图必须随追加更新（「截图随追加」——UPG-94 logo 追加未更新截图教训）");
  push("");
  push("## 七、登记说明");
  push("");
