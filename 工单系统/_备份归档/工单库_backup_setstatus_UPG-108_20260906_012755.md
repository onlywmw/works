...
std: —
delivery_id: —
designer: ❌ 已销单 @2026-09-05（用户拍板+设计师评估）
dev: —
inspector: —
merge: 销单
actor: 设计师
updated_at: 2026-09-05T21:07:20
```

**状态**：✅ 方案完成待派单 ｜ **优先级**：P2 ｜ **出单人**：设计师 ｜ **日期**：2026-08-27 → ❌ **销单 @2026-09-05（用户拍板+设计师评估）**：核心需求「资产统一档案+到期提醒」已被事实覆盖（cert 查表制 README+域名登记表+处理中心资产档案表.md+云服务器总览台账）；剩余仅后台画册 UI——按「先官网后后台」原则不成立，销卡
...
merge: 销单
actor: 设计师
updated_at: 2026-09-05T21:07:28
```

**状态**：✅ 方案完成待派单（依赖 W-06 之后施工，见下）｜ **优先级**：P2 ｜ **出单人**：设计师 ｜ **日期**：2026-08-27 → ❌ **销单 @2026-09-05（用户拍板+设计师评估）**：仪表盘四组件需求已被 SYS-02 阶段四 E4 全景看板覆盖（桌面看板.bat 已落地）；剩余仅事件日历视图一组件——不足以养卡，事务线真需要时重立聚焦单
...
std: —
delivery_id: —
designer: ❌ 已销单 @2026-09-05（用户拍板——目标事实达成：备案+官网在线+容器面已部署）
dev: —
inspector: —
merge: 销单：域迁移目标已由演进达成，卡面流程未施工
actor: 设计师
updated_at: 2026-09-05T20:50:39
```

**状态**：✅ 方案完成待派单（**硬阻塞：批 0 mov-ai.cn ICP 备案未落地（事件 E-03），备案前不得对生产切换——可在测试路径预演**）｜ **优先级**：P1 ｜ **出单人**：设计师 ｜ **日期**：2026-08-27 ｜ **出卡**：2026-08-28（断电重排补登）→ ❌ **销单 @2026-09-05（用户拍板）**：目标已事实达成（非按卡施工）——ICP 备案已上页脚（c5bb94af 已合）+ mov-ai.cn 官网在线运行+容器面已部署（llms.txt/sitemap 200 实证 @2026-09-05）+ mow.kim 退居后台保留服务；卡面规划的「批 0 备案→切换→API 双跑兜底」流程被实际演进取代，本卡销项；残留事实口径=项目配置 §一之二域名分工
...
inspector: —
merge: ✅ 已合 main @489398f6 + 已部署生产 @2026-09-05（双域 /tools+mov-ai-public 实证 sub
actor: 设计师
updated_at: 2026-09-05T20:47:36
```
...
merge: 销单
actor: 设计师
updated_at: 2026-09-05T21:07:37
```

**状态**：✅ 方案完成，待派单 ｜ **优先级**：P3（小修批，顺队列）｜ **出单人**：设计师 ｜ **日期**：2026-08-27 → ❌ **销单 @2026-09-05（用户拍板+设计师评估）**：内容为 workbench（后台线）真 bug 清单——按分工原则现在不做；bug 清单+修复口径已转挂账登记表保留（挂账-w12-workbench前端bug清单），后台线启动时转工单
...
designer: 📌 已派单（用户直接外派）
dev: ✅ C 完成 @2026-09-05（f5e7212a——站点三件+3 端点+契约锚；DEL 挂起已随 STD 补冻解锁；待验收）
inspector: —
merge: 改向挂起：试点职能由 UPG-105 批二承接，工单三工具留商家后台线
actor: 设计师
updated_at: 2026-09-05T10:13:04
```
...
**状态**：❌ **已作废 @2026-09-03**（注明：原“已取消”，E4 终态词统一为作废（用户拍板「只要两种模式：经典+极简，极简只是画面极简不阉割能力」——5→2 模式收敛转 UPG-84：code 模式退役，本单修复对象消失，根因消除优于补丁；STD-UPG-83-v1 冻结留档不删）｜ ~~优先级：P1~~ ｜ **优先级**：P1（作废留档）
...
→ 🔄 **认领+盘点完成 @2026-09-05（程序员 Claude/wmw0027 · WIP 未动产品代码）**：①**段①资产锚全定位**：Evaluator.kt(91 行 六指标 evaluate 形态)/EvalFixture.kt(139 行 13 例 c01-c13[NO_CALL×2 已有]/EvalCase 字段 expectDecision/expectTools/expectBlocked/expectConfirm/expectNoCall)/EffectSpecs.kt(SideEffect{PURE,READ,WRITE}+Env{LOCAL,EXTERNAL}+EffectSpec+ToolDecisionResult/PlanVerdict——**L0-L3 分级层待建**) /OrchRules+OrchRuleEngine/DagPlanner(244 行 mockCostMs+effectOf)/TraceRecord.memoryCandidate ToolOrchModel:97（toJournalMap:118 已序列化；**恒空串填充点=ToolOrchestrator:352**）②**A1-A4 施工蓝图**：A1=EvalFixture.cases 13→25+（NO_CALL+2/MULTI_CALL+2/边界+2——**评测对象=ToolOrchestrator 关键词决策器 baseline 不动**，新用例须与其行为一致）；A2=**SafetyLevel L0-L3 映射层新建**（SideEffect×Env→L 级：L0 纯本地读/L1 本地写/L2 外部读/L3 外部写敏感）+**own 兜底分类器**（EffectSpecRegistry 未登记→保守 L2+）+L2/L3→REQUIRED 走现有审批通道；A3=**PlanMode{PARALLEL,SEQUENTIAL,CONDITIONAL} 新建**（现无 mode 枚举）+PlanExecutor（DagPlanner 分层→按 mode 执行→结果回填）+互斥/优先级（EffectSpecs 判据）；A4=memoryCandidate 实值（工具结果→候选文本）→memoryOsSemantic.createProposal(actor=ai-proposal)→PROPOSED（UPG-52 四态机+UPG-107 payload 链路对接——**全链零自动 ACTIVE**）③**变异 5 锚落点**：A1 删 NO_CALL 正例→noCall 指标红/A2 绕确认门→定向红/兜底恒 L0→未登记分级红/A3 Parallel 塌缩→模式红/A4 直写 ACTIVE→回流红
...
**交付**：卡：UPG-104；STD：STD-UPG-104-v1（已冻结 @2026-09-05 sha=e6d8a7f9）
...
→ 🔄 **认领+盘点完成 @2026-09-05（程序员 Claude/wmw0027 · WIP 未动产品代码）**：①**架构异议结论=不推翻派单文**——App 内实现+**ForegroundService** 兑现设计稿「独立 Service 保活」意图（投影数据源零 IPC 成本+官方保活折中）；独立**进程**留阶段 2 评审项 ②**盘点实录**：MiniHttpServer.kt(162 行) 绑定 127.0.0.1 硬编码[:48]——UserProxy 需 bindHost 参数化（默认回环不破坏 8389 锚）；McpServer token 鉴权形态可参照；扫码=**LightOcr 无 QR 解码**——QR 基建待建（ZXing/MLKit barcode 二选一）；InfoVault.list():291 白名单投影 ✓/cred.<platform> 凭据 NEVER ✓；RoomStore.listRooms():62 ✓；**衣柜无独立模块**（CapsuleResolver 命中为别义）——衣柜=InfoVault 类目子集语义待设计稿核 ③**续作路径**（新会话）：投影数据源定稿→UserProxyService(ForegroundService+bindHost 0.0.0.0:8841+token 鉴权)→信令端点(account-service /pair/* 内存态零存储)→官网 /user/ 页→扫码基建→真机双端 U108-1~5
...
# UPG-110 官网容器面·批二 A 通道（MCP Streamable HTTP 活端点）
**分类**：M4 工具/MCP ｜ 标签：M3 平台/基建
...
head: 1df4a3e3
std: STD-UPG-110-v1
delivery_id: DEL-UPG110-20260905-001
designer: 已派单（批二 A 通道，STD-UPG-110-v1 已冻结 99b78881）
dev: ✅ C 完成 @2026-09-05（1df4a3e3，2 文件+401；生产已上线实证）
inspector: ✅ 复验 §P61 + 终态复核 §P61-T 通过 @2026-09-05（合入实证+5/5 实物双端 sha+生产 200 同源——审验员独立亲验）
merge: 已合 main @eebbd57a + 终态复核闭环 @2026-09-05（§P61-T 全生命周期确认；P3 勘误 952225f1→bdfa4f3a 已补 acef09e5）
actor: 设计师
updated_at: 2026-09-05T23:40:00
```

**状态**：📌 **已立卡·已派 @2026-09-05**（UPG-105 批一已合 main[841f591d] 后按约拆出；设计依据=容器面设计 v3 §4.5 验证层/§4.6 A 通道运营/§5 工程边界；**溯源复核已过 @2026-09-05**：官网服务形态=market-web/ 静态+Caddy/Node 服务在案，直接先例=`market-web/scene-mcp/server.mjs`[12306 MCP server：JSON-RPC tools/list+tools/call+统一错误信封]——A 通道=同形态新模块；批一产物[llms.txt/tools-schema.json/sync-manifest.json/capability-registry 同源链]全部在 main 可复用）｜ **优先级**：P1 ｜ **出单人**：设计师 ｜ **日期**：2026-09-05 → ✅ **验收员复验 通过 @2026-09-04**（ACCEPTANCE_LOG §P61：1df4a3e3 隔离 worktree——**生产 curl 三连亲测**[tools/list 六工具+mov_site_info ICP 备案+错误路径 -32602+限流确认未触发+缓存头]+**生产同源对账逐字段 ALL MATCH**[4 能力 7 字段 vs 仓内 registry]+server.test.mjs 7/0 亲跑确认+L3 只读零写；coverage FULL；**达待合→审验员→设计师**） → 🏁 **已合 main·终态复核通过 @2026-09-05**（ACCEPTANCE_LOG §P61-T：origin/main eebbd57a 合入；5/5 交付物独立亲验+server.mjs sha 双端一致 15659065d4493777[main=1df4a3e3 逐字节]——verify-hash 终态「文件在 main+内容一致」双实证；生产 mov-ai.cn/mcp/ 同源闭环）

## 标题
mov-ai.cn 上线 MCP Streamable HTTP（2026-07-28 规范）活端点：六工具（mov_site_info/capabilities/tools/search/market_catalog/guide）从「schema 静态描述」变「AI 可直接调用」——官网双层设计层 1 收官件 + WebMCP 对外名片正主。

## 施工（v3 §4.5/4.6/5 口径）
1. **端点模块**：`market-web/` 下新建容器面 MCP 服务（照 scene-mcp/server.mjs 形态：JSON-RPC tools/list+tools/call+统一错误信封）——六工具实现，数据源=capability-registry.json 同源（禁手抄，读 registry 生成）；
2. **验证戳**：所有工具返回 registry_version + last_sync_time（与 sync-manifest.json 同源；发布联动重生成）；
3. **A 通道运营**：UA 约定（`MOV-AI-Client/1.0`，统计/审计用不强制）+ 响应 CDN 缓存 5min 头 + 限流锚（IP+UA 组合，第一版不建 token 体系）；
4. **部署**：服务器 systemd 服务 + Caddy 反代路径挂载（/mcp 或设计定路径），上生产。

## 红线
只读（查询不落库不改状态）；A 面零凭据/会话/用户隐私；install_url 一律指向人操作界面（AI 不代替人安装）；诚实空态；同源生成（六工具数据必须读 registry，手抄=打回）。

## 验收
STD-UPG-110-v1（L1 契约测试+L2 生产 curl 实证+3 变异锚；判据以冻结区为准）。

## 派单交接段
认领 worktree=mov-upg110 branch=feat/upg110（基最新 origin/main）；完成后登记两个表；报告 `程序员\交付报告\DELIVERY_UPG110_*.md`；部署需重启服务端（告知用户窗口）；交付含《共享面影响清单》+Token/KV 两节；manifest deliver-gen 机制产出+verify-hash HASH_OK 再登记。

# UPG-111 契约测试死角扫尾（UPG-100 A3 结论 18 处——Appearance 族优先）

**分类**：M3 平台/基建 ｜ 标签：M8 UI/交互、M2 体系/治理
...
## 背景
UPG-100 交付时 A3 抽检静态扫 8 个契约测试文件 → 18 处疑似死角（Appearance 16 最多 + Approval 2）；方法论（已知坑.md #10）已入库。

## 范围（建议）
- Appearance 族 16 处：按 UPG-100 锚②方法（计数+局部性）升级——优先
- Approval 族 2 处：次之
- 方法论：已知坑.md #10（A2 同源）

## 不做
- 不重写行为级测试（WebView 回路真机持有）
- 不删测试（只升锚）

## 验收（简式）
- U-111-1 升级后每处锚=计数+局部性（非 contains 单点）
- U-111-2 变异亲杀：删真实读点→红
- U-111-3 全量零失败回归
