...
**状态**：✅ 方案 v1 定稿（用户调研后拍板「先修地基」）｜ 未派单 ｜ **优先级**：P2
→ **依据**：2026-09-02 调研 GitHub 成熟方案（Vant4/Ark+Uno/shadcn-vue）后，用户拍板先修地基——病根=双写目录/形态类分散/无运行时/10 入口割裂/规范脱节（本卡=架构地基，不替代 UPG-50 阶段 2）
...
- A1（本单核心）：`前端设计/mov-vue/src/catalog/ui-components.json` 单一目录源 + `scripts/gen-ui-catalog.mjs` 生成 Kotlin `UiComponentCatalog.kt` + Vue `COMPONENTS` 段 + 断言脚本（防手改漂移、亲杀测试）；双写归零。
- A2：SETTINGS 族形态类从 SettingsPage.vue scoped 迁入 tokens.css 形态层；规范与 token 对账（--primary 等漂移项处置）；产出 `tools/ui-catalog-audit.mjs` 未接入页面清册。
- A3（B 阶段）：统一页面宿主（WebView/桥/主题注入共享）——A2 验收后再拆单。
- 不做：不引 Vant/Ark/UnoCSS（下一阶段另议）；不改 20 条编号/52 档数据；不改组件视觉（收拢版 UI 已合 main 不动）。

## 验收锚

- A1-1：唯一目录源（grep 断言无第二份目录数据）；A1-2：改 schema 重新生成→Kotlin+Vue 同步；A1-3：手改生成物→断言红。
- A2-1：SETTINGS 形态类在 tokens.css、scoped 无残留；A2-2：漂移项对账表产出并处置。
- A3：宿主共享后 10 页零回退（抽查 3 页打开正常）——B 阶段验收。

**红线**：登记=设计师单写点；xlsx 生成物禁手改（sync 生成）；生成物带「DO NOT EDIT」头；不破坏 UPG-50 阶段 2 已派工作。

**交付**：方案文档 `01_外观/UI组件化改造_地基_方案_v1_2026-09-02.md`；卡：UPG-70
