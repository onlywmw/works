...
**状态**：✅ 方案 v3 定稿（输入与MCP工具联动机制_设计_v3，冻结，大神 9.3+Trace契约）→ 🔨**已派单 @2026-08-30**（用户转派；worktree=mov-tool-orch branch=feat/tool-orch）——待程序员认领 ｜ **优先级**：P0 ｜ **出单人**：设计师 ｜ **日期**：2026-08-30→ 📋 **派单文已补 @2026-09-02**（——D1-D4 判据+范围+基线+验收；**P0 可认领**：任意线先到先得）→ 📝 **设计 v3.1 补丁 @2026-09-02**（Plan 模式=顺序真相层：两段式先 Plan 后执行+steps DAG+机器校验+顺序可见——用户定「plan 没做好联动必乱」；；并入 UPG-46 施工口径——**P0-4 三模式由 dependsOn 显式表达**）
**卡点**：已派·待认领——无 ｜ 
