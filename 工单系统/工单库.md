...
**状态**：✅ 方案 v2 定稿（2026-09-02：v1 经大神外部评审全部采纳——5 补强+3 实施前置）｜ 未派单 ｜ **优先级**：P2
→ **依据**：2026-09-02 调研 GitHub 成熟方案（Vant4/Ark+Uno/shadcn-vue）后，用户拍板先修地基；大神评审确认可执行，采纳：本轮不建组件运行时/A2 先影响面评估+用户裁决/生成链入 CI（DO NOT EDIT+pre-commit+CI diff）/schema 加 version+注释（不添半截运行时字段）/A3 试点先行拆小单；口径=`UI组件化改造_地基_方案_v2_2026-09-02.md`
...
- **明确边界**：本轮**不建设组件运行时/注册机制**（病根 3 不处置=有意为之，可插拔运行时归后续阶段另立卡）；本卡只统一**目录**与**形态层**。
- A1（本单核心）：`前端设计/mov-vue/src/catalog/ui-components.json` 单一目录源（`version`+字段注释；semanticType/provider=现有目录元数据、不新增半截运行时字段）+ `scripts/gen-ui-catalog.mjs` 生成 Kotlin `UiComponentCatalog.kt` + Vue `COMPONENTS` 段 + 断言；防漂移三层=DO NOT EDIT 头+pre-commit+CI `gen-ui-catalog && git diff --exit-code`；双写归零。
- A2：SETTINGS 族形态类从 SettingsPage.vue scoped 迁入 tokens.css 形态层；**先影响面清单 → 用户裁决**（改代码对齐规范=视觉回归风险+回归测试｜改规范对齐实物=标「暂不调整」）；Motion/z/elevation/inset token 落库或删除；产出 `tools/ui-catalog-audit.mjs` 未接入页面清册 + 对账表（含处置结果+影响面）。
- A3（B 阶段）：**试点先行**——先 SettingsSheet 做 PageHost 抽象试点（WebView+AssetLoader+Bridge+主题注入），试点结论→拆多小单逐步替换；不一次重构 10 页。
- 不做：不引 Vant/Ark/UnoCSS（下一阶段另议）；不建设组件运行时；不改组件视觉（A2 token 对齐例外——须先影响面评估+用户裁决）；不改 20 条编号/52 档数据。

## 验收锚

- A1-1：唯一目录源（grep 断言无第二份目录数据）；A1-2：改 schema 重新生成→Kotlin+Vue 同步；A1-3：手改生成物→断言红（亲杀）；**A1-4**：CI `gen-ui-catalog && git diff --exit-code` 通过（证据落交付报告）。
- A2-1：SETTINGS 形态类在 tokens.css、scoped 无残留；**A2-2**：影响面清单（消费待对齐 token 页面/组件+差异说明）；**A2-3**：对账表三列（漂移项/处置结果/影响面）。
- **A3-0**：PageHost 试点结论文档（SettingsSheet）——通过才拆小单。
- 登记=设计师单写点；xlsx 生成物禁手改（sync 生成）；不破坏 UPG-50 阶段 2 已派工作。

**红线**：登记=设计师单写点；xlsx 生成物禁手改（sync 生成）；生成物带「DO NOT EDIT」头；不破坏 UPG-50 阶段 2 已派工作。

**交付**：方案文档 `01_外观/UI组件化改造_地基_方案_v2_2026-09-02.md`（v1 归档留指针）；卡：UPG-70
