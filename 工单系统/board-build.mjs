...
async function stageChange(sel){
  const no = sel.dataset.noSelect;
  const v = sel.value;
  if(!v){ return; }
  try{
    let j;
    if(v === "reject"){ if(!confirm(no + " 改回施工·回炉？")){ sel.value=""; return; } j = await BoardAPI.reject(no); }
    else if(v === "restore"){ if(!confirm(no + " 恢复（取消回炉）？")){ sel.value=""; return; } j = await BoardAPI.restore(no); }
    if(j && j.ok){
      sel.value = "";
      if(!updateRowLocally(no, v === "reject" ? "rejected" : "delivering", v === "reject" ? "回炉重修中" : "已恢复——施工中")) location.reload();
      alert((v === "reject" ? no + " 已提交回炉" : no + " 已恢复") + "（" + (j.data?.txid || "") + "）");
    } else {
      sel.value = "";
      alert("状态更新失败：" + ((j && (j.code || j.msg)) || "未知"));
    }
  }catch(e){ sel.value = ""; alert("状态更新失败（服务未运行？）：" + e.message); }
}
